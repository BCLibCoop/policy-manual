...
Abort Transit
^^^^^^^^^^^^^
Abort transit should only be done by source/lending library. Abort transit deletes the transit record,  and should never be done at the destination/borrowing library.
