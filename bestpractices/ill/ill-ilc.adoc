...
There are a few templates in Sitka_templates > Intra-federation ILL.

The following two templates are designed for libraries to generate monthly ILC hold statistics:

* https://docs.libraries.coop/sitka/_report_definition_examples.html#_library_inbound_ilc_holds_count_for_selected_month[LIBRARY: Inbound ILC holds count for selected month]

* https://docs.libraries.coop/sitka/_report_definition_examples.html#_library_outbound_ilc_holds_count_for_selected_month[LIBRARY: Outbound ILC holds count for selected month]
...
In Sitka_templates > Circulation > Overdue folder, there are templates for libraries
to track their items lent to other libraries, for example the template called _Overdue Items Checked out at Other Libraries_.
