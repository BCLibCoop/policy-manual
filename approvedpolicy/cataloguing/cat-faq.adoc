...
Send a request to Co-op Support. Support assesses potential sources based on perceived need by Sitka libraries and quality of records.

Please note, not all organizations provide Z39.50 access to their database.
...
The following are guidelines for using RDA records in the system:
...
* We want to minimize duplication of records; use the RDA record if already in the system, but do not import a duplicate record in RDA if an AACR2 record for the same title already exists in Sitka. Instead, attach your copies to the AACR2 record.
